...
/// 保存单条技能（upsert）。
pub async fn save_skill(app: &AppHandle, skill: &Skill) {
    let p = pool(app);
...
/// 删除单条技能。
pub async fn delete_skill(app: &AppHandle, id: &str) {
    let p = pool(app);
...
/// 启动时加载所有技能。
pub async fn load_all_skills(app: &AppHandle) -> Vec<Skill> {
    let p = pool(app);
    let rows = sqlx::query(
