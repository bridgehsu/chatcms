...
pub async fn generate(
    ...
    #[derive(serde::Deserialize)]
    struct VideoJobError { message: Option<String> }
    ...
    save_bytes(app, bytes, prompt, model, size, seconds, Some(remote_id)).await
}
...
pub async fn update(
    ...
    let mut videos = repo::load_all(app).await;
...
pub async fn delete(app: &AppHandle, id: String) -> Result<()> {
    let videos = repo::load_all(app).await;
