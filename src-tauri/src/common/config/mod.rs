...
pub struct GeneralSettings {
    ...
    /// 是否为模型请求启用 HTTP 代理（需同时配置 http_proxy_url）；默认关闭直连
    #[serde(default = "default_false")]
    pub use_system_proxy: bool,
    /// 代理地址，如 http://127.0.0.1:7890（仅 use_system_proxy=true 时生效）
    #[serde(default)]
...
pub struct AppConfig {
    ...
    /// 权限：模式 / 域策略 / MCP / 约束
    #[serde(default)]
    pub permission: crate::common::permission::PermissionConfig,
...
impl Default for AppConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            permission: crate::common::permission::PermissionConfig::default(),
...
impl AppConfig {
    ...
    pub fn hydrate_general_from_legacy(&mut self, app: &tauri::AppHandle) {
        ...
        if self.general.video_base_url.trim() == defaults.video_base_url {
            if let Some(v) = crate::common::persist::load_mpt_config(app) {
        ...
        if self.general.crawler_base_url.trim() == defaults.crawler_base_url {
            if let Some(v) = crate::common::persist::load_crawler_config(app) {
                if let Some(url) = v.get("base_url").and_then(|x| x.as_str()) {
                    let url = url.trim().trim_end_matches('/');
                    if !url.is_empty() {
