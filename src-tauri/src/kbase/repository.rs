...
use super::{KnowledgeEntry, KnowledgeSiteProfile};
use crate::db::DbPool;
...
pub async fn save_entry(app: &AppHandle, entry: &KnowledgeEntry) {
    let p = pool(app);
...
pub async fn delete_entry(app: &AppHandle, id: &str) {
    let p = pool(app);
...
pub async fn load_all(app: &AppHandle) -> Vec<KnowledgeEntry> {
    let p = pool(app);
    sqlx::query(
...
pub async fn save_site_profile(app: &AppHandle, profile: &KnowledgeSiteProfile) {
    let p = pool(app);
    let _ = sqlx::query(
...
pub async fn load_site_profile(app: &AppHandle) -> KnowledgeSiteProfile {
    let p = pool(app);
